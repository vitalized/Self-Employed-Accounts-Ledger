...
export async function registerRoutes(
  ...
  app.get("/api/gamification/user-achievements", async (req, res) => {
    try {
      const userAchievements = await storage.getUserAchievements();
  ...
  app.get("/api/gamification/challenges", async (req, res) => {
    try {
      ...
      const userChallenges = await storage.getUserChallenges();
      const userChallengeMap = new Map(userChallenges.map(uc => [uc.challengeId, uc]));
      
      const challengesWithProgress = allChallenges.map(c => ({
        ...c,
        currentValue: userChallengeMap.get(c.id)?.currentValue || 0,
        isCompleted: userChallengeMap.get(c.id)?.isCompleted === 1,
        started: userChallengeMap.has(c.id)
      }));
  ...
  app.post("/api/gamification/challenges/:id/start", async (req, res) => {
    try {
      const challenge = await storage.startChallenge(req.params.id);
      res.json(challenge);
    } catch (error) {
  ...
  app.get("/api/gamification/stats", async (req, res) => {
    try {
      ...
      const completedChallenges = userChallenges.filter(uc => uc.isCompleted === 1).length;
      
      res.json({
        ...stats,
        achievementsUnlocked: userAchievements.length,
        challengesCompleted: completedChallenges,
        pointsToNextLevel: ((stats.level) * 100) - stats.totalPoints
      });
    } catch (error) {
  ...
  app.post("/api/gamification/check-achievements", async (req, res) => {
    try {
      const transactions = await storage.getTransactions();
      const mileageTrips = await storage.getMileageTrips();
      const rules = await storage.getRules();
      const allAchievements = await storage.getAchievements();
      
      const categorizedTransactions = transactions.filter(t => t.type !== 'Unreviewed');
      const unreviewedTransactions = transactions.filter(t => t.type === 'Unreviewed');
      const businessExpenses = transactions.filter(t => t.type === 'Business' && t.businessType === 'Expense');
      const totalExpenses = businessExpenses.reduce((sum, t) => sum + Math.abs(parseFloat(t.amount)), 0);
      const netProfit = transactions
        .filter(t => t.type === 'Business')
        .reduce((sum, t) => {
          const amount = parseFloat(t.amount);
          return t.businessType === 'Income' ? sum + amount : sum - Math.abs(amount);
        }, 0);
      
      const newlyUnlocked: string[] = [];
      
      for (const achievement of allAchievements) {
        const hasIt = await storage.hasAchievement(achievement.id);
        if (hasIt) continue;
        ...
        switch (achievement.code) {
          case 'first_transaction':
            shouldUnlock = categorizedTransactions.length >= 1;
            break;
          case 'transactions_10':
            shouldUnlock = categorizedTransactions.length >= 10;
            break;
          case 'transactions_50':
            shouldUnlock = categorizedTransactions.length >= 50;
            break;
          case 'transactions_100':
            shouldUnlock = categorizedTransactions.length >= 100;
            break;
          case 'transactions_500':
            shouldUnlock = categorizedTransactions.length >= 500;
            break;
          case 'all_categorized':
            shouldUnlock = unreviewedTransactions.length === 0 && categorizedTransactions.length > 0;
            break;
          case 'first_mileage':
            shouldUnlock = mileageTrips.length >= 1;
            break;
          case 'mileage_10':
            shouldUnlock = mileageTrips.length >= 10;
            break;
          case 'first_rule':
            shouldUnlock = rules.length >= 1;
            break;
          case 'rules_5':
            shouldUnlock = rules.length >= 5;
            break;
          case 'tax_savings_1k':
            shouldUnlock = totalExpenses >= 1000;
            break;
          case 'tax_savings_5k':
            shouldUnlock = totalExpenses >= 5000;
            break;
          case 'tax_savings_10k':
            shouldUnlock = totalExpenses >= 10000;
            break;
          case 'profit_positive':
            shouldUnlock = netProfit > 0;
            break;
        }
        
        if (shouldUnlock) {
          await storage.unlockAchievement(achievement.id);
          await storage.addPoints(achievement.points);
